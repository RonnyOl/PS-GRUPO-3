...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const endpoint = isLogin ? 'http://localhost:8080/v1/auth/login' : 'http://localhost:8080/v1/auth/registerdev';

            const payload = isLogin
                ? { email: formData.email, password: formData.password }
                : { name: formData.name, email: formData.email, password: formData.password, studioName: formData.studioName, taxCode: formData.taxCode };
            ...
            if (response.status === 200) {
                if (isLogin) {
                    ...
                    setTimeout(() => {
                        setIsLogin(true);
                        setFormData({ name: '', email: '', password: '', confirmPassword: '', studioName: '', taxCode: '' });
                        setFeedback({ type: '', message: '' });
                    }, 1800);
                }
            } else {
        ...
        <div className="min-h-screen bg-[#0b0e14] flex items-center justify-center p-4 md:p-8 relative overflow-hidden select-none">
            ...
            <div className="w-full max-w-md bg-[#121824]/60 border border-white/[0.07] backdrop-blur-2xl rounded-2xl shadow-[0_25px_60px_-15px_rgba(0,0,0,0.7)] p-8 md:p-10 z-10 transition-all duration-300 hover:border-white/[0.12] hover:shadow-[0_25px_60px_-15px_rgba(102,192,244,0.05)]">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                    ...
                    {!isLogin && (
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block pl-1">Nombre del Estudio</label>
                            <div className="relative">
                                <input
                                    type="text"
                                    name="studioName"
                                    ...
                                    onChange={handleChange}
                                    placeholder="ej. Studio One"
                                    className={`w-full bg-[#0a0d14]/70 border ${errors.name ? 'border-rose-500/50' : 'border-white/[0.08] hover:border-slate-500/50'} focus:border-[#66c0f4] focus:ring-4 focus:ring-[#66c0f4]/15 rounded-xl px-4 py-3 pl-11 text-sm text-white placeholder-slate-600 outline-none transition-all`}
                                />
                                <User className="absolute left-4 top-3.5 w-4 h-4 text-slate-500" />
                            </div>
                            {errors.studioName && <span className="text-[10px] text-rose-400 block pl-1">{errors.studioName}</span>}
                        </div>
                    )}

                    {!isLogin && (
                        <div className="space-y-1.5">
                            <label className="text-[10px] font-bold text-slate-400 uppercase tracking-widest block pl-1">Tax code</label>
                            <div className="relative">
                                <input
                                    type="text"
                                    name="taxCode"
                            ...
                            {errors.taxCode && <span className="text-[10px] text-rose-400 block pl-1">{errors.taxCode}</span>}
                        </div>
                    )}
